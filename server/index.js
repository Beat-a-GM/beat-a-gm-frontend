...
const { filterMovesWhite, filterMovesBlack } = require('./filterMoves');
...
axios.get(apiUrl)
  .then(response => {
    const regex = /\b1\..*?(?:0-1|1-0)\b/g;
    const matches = response.data.match(regex).slice(0, 10);
    //increase 10 in order to get more of hikaru's games in october
    for (let i=0;i<matches.length;i++) {
      const moveOrderWhite = filterMovesWhite(matches[i]);
      const moveOrderBlack = filterMovesWhite(matches[i]);
